...
class PluginMycustomviewMyview5 extends CommonDBTM
{
   ...
   public function getTabNameForItem(CommonGLPI $item, $withtemplate = 0)
   {
      ...
      $result = $DB->query("SELECT * FROM glpi_groups WHERE id = $result->groupe_id_5")->fetch_object();
   ...
   static function displayTabContentForItem(CommonGLPI $item, $tabnum = 1, $withtemplate = 0)
   {
      if ($item->getType() == 'Central') {
         ...
         global $PLUGIN_HOOKS, $DB;
         /*$count = 0;
         $result = $DB->query("SELECT * FROM glpi_groups_users WHERE users_id = 2");
            while ($data = $DB->fetchArray($result)) {
                if(!empty($data['id'])) $count++;  
            }
